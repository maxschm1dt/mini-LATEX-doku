...
\subsection{Von Overleaf zu lokal wechseln}
...
Um \textbf{lokal auf deinem PC} ein Dokument zu erstellen (\autoref{Installation} notwendig), einfach einen neuen Ordner erstellen und dann in \textbf{VS-Code}
eine neue Datei mit der Endung \verb|.tex| erstellen.

Folgender \LaTeX -Code ist eigentlich immer ein guter Start:
\begin{lstlisting}
...
In \textbf{Overleaf} kann mit dem \textbf{grünen (Re-)Compile} oder auch mit \verb|Strg + S| das PDF erstellt bzw. aktualisiert werden.



In \textbf{VS-Code} kann mit dem \textbf{grünen Pfeil} oben rechts das Dokument das erste Mal kompiliert (d.h. das PDF erstellt) werden; ab dann kann immer mit \verb|Strg + S| gespeichert und kompiliert werden.
...
\noindent Wollen eine eher ordentlicher Darstellung mit Bildunterschrift, können wir eine \textit{figure} benutzen:
...
\begin{lstlisting}
    \begin{figure}
        ...
        \label{figure-passenderName} %ein label mit einem einzigartigem Namen um die Abbildung spaeter Referenzieren zu koennen
    \end{figure}
\end{lstlisting}
Später im Text kann man dann so auf die Abbildungsnummer zugreifen:
\begin{lstlisting}
    siehe Abbildung \ref{figure-passenderName}
\end{lstlisting}

\noindent Mit dem Package \textit{float} und der Option \textit{H} kann mann sicher stellen, dass die Darstellung genau an der Stelle im Dokument erscheint, in der sie auch im Code ist: dies kann aber sehr schnell zu Formatierungsproblemen führen.
...
\sectionwithsubtitle{Mathezauber}{Was man im Infostudium so an Sonderzeichen braucht}
...
\begin{figure}[H]
	...
	\begin{tabular}{l | c | l}
        ...
        Betrag         & $ \lvert x \rvert $                 & \lstinline|$ \lvert x \rvert $|                 \\
	\end{tabular}
\end{figure}
...
\subsection{Graphen}
Mit \lstinline|\usepackage{tikz}| kann man solche Graphen basteln:
\begin{figure}[h]
    \centering
    \begin{tikzpicture}[node distance={15mm}, main/.style = {draw, circle}] 
        ...
        \draw[<->] (1) -- node[midway, above right, sloped, pos=0.1] {50} (2);
        \draw[->] (2) -- node[midway, above right, sloped, pos=0.2] {10} (3);
        \draw[->] (3) -- node[midway, above right, sloped, pos=0.5] {1} (4);
        \draw[->] (1) to [out=270, in=270, looseness=2] node[midway, below right, sloped, pos=0.5] {40} (4);
        \draw[->] (5) -- node[midway, above right, sloped, pos=0.8] {133} (2);
        \draw[<->] (5) -- node[midway, above right, sloped, pos=0.8] {1} (4);
    \end{tikzpicture}
\end{figure}
...
\begin{lstlisting}
    \begin{tikzpicture}[node distance={15mm}, main/.style = {draw, circle}] 
        \node[main] (1) {$g_1$}; 
        \node[main] (2) [above right of=1] {$g_2$}; 
        \node[main] (3) [below right of=1] {$g_3$}; 
        \node[main] (4) [below right of=2] {$g_4$};
        \node[main] (5) [above right of=4] {$s$};
